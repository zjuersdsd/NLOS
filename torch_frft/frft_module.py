...
def frft_shifted(fc: torch.Tensor, a_param: Union[float , torch.Tensor], *, dim: int = -1) -> torch.Tensor:
    return fftshift(frft(fftshift(fc, dim=dim), a_param, dim=dim), dim=dim)


def ifrft(fc: torch.Tensor, a_param: Union[float , torch.Tensor], *, dim: int = -1) -> torch.Tensor:
    return frft(fc, -a_param, dim=dim)


def frft(fc: torch.Tensor, a_param: Union[float , torch.Tensor], *, dim: int = -1) -> torch.Tensor:
    N = fc.size(dim)
    if N % 2 == 1:
...
def _bizinter_real(x: torch.Tensor, *, dim: int = -1) -> torch.Tensor:
    ...
    xf = torch.index_fill(xf, dim, torch.arange(N1, N2, device=xf.device), 0)
    xf = 2 * torch.real(ifft(xf, dim=dim))
